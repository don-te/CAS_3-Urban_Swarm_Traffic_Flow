...
class Visualizer:
    def __init__(self, bounds):
        pygame.init()
        # Slightly wider window for the sidebar
        ...
        # --- UI STATE ---
        self.is_paused = True  # <--- CHANGED: Start in PAUSED state
        self.sim_speed = 1.0
        ...
        # 1. Play/Pause Button
        # Initial State: Green "START" button
        self.play_btn = Button(
            x=panel_x, y=50, w=100, h=40,
            text="START",       # <--- CHANGED: Initial text
            callback=self.toggle_pause,
            color=(60, 200, 60),      # <--- CHANGED: Green (Safe/Go color)
            hover_color=(80, 230, 80)
        )

        # 2. Agent Count Slider
    ...
    def toggle_pause(self):
        ...
        if self.is_paused:
            # If we just paused, show "RESUME" or "PLAY"
            self.play_btn.text = "RESUME"
            self.play_btn.color = (60, 200, 60) # Green
            self.play_btn.hover_color = (80, 230, 80)
        else:
            # If we are running, show "PAUSE"
            self.play_btn.text = "PAUSE"
            self.play_btn.color = (200, 60, 60) # Red
    ...
    def draw(self, city, rickshaws):
        ...
        title_surf = title_font.render("CONTROLS", True, (150, 150, 150))
