...
class Visualizer:
    def __init__(self, bounds):
        ...
        self.is_paused = True
        self.sim_speed = 1.0
        self.path_constant = True # Default: True (Same path)
    ...
    def toggle_path_mode(self):
        ...
        if self.path_constant:
            ...
            self.path_btn.text = "PATH≠CONST"
            self.path_btn.color = (255, 0, 0) # Orange
    ...
    def handle_ui_events(self, event):
        ...
        self.path_btn.handle_event(event) # Handle Path Toggle
    ...
    # ... (Rest of drawing methods: draw_traffic_lights, _draw_stop_line, draw) ...
    # Be sure to keep draw_traffic_lights, _draw_stop_line and the rest of the draw method unchanged
    # except for the sidebar drawing part which I will include below for completeness of UI layout
    ...
    def draw(self, city, rickshaws, total_collisions, history_list, traffic_manager=None):
        ...
        def to_screen(lat, lon):
            return map_coords_to_screen(lat, lon, min_lat, max_lat, min_lon, max_lon, c.SCREEN_WIDTH, c.SCREEN_HEIGHT)
        ...
        # Lights
        if traffic_manager: self.draw_traffic_lights(city, traffic_manager, to_screen)
        ...
        self.path_btn.draw(self.screen) # Draw Path Toggle
        self.agent_slider.draw(self.screen)
        self.speed_slider.draw(self.screen)
        
        # Stats
        pygame.draw.line(self.screen, (60, 60, 60), (20, 310), (self.sidebar_width - 20, 310), 1)
